...
///
/// The endpoint reports its input with `extended`, which parks the result in
/// the endpoint's status entry for the circuit thread to count as processed.
/// Both `disconnect_input` (an ad hoc `INSERT` disconnects its endpoint as soon
/// as its queue drains) and the reply itself can land while that result is
/// still parked, and the endpoint's status entry -- with the parked result --
/// used to go away with the endpoint.  The records reach the circuit either
/// way, so losing their count left `total_processed_records` permanently below
/// `total_input_records`, and a client waiting for the pipeline to catch up
/// with its write waited forever.
#[test]
